...
export const computeStats = (
  ...
  const shouldHideNormalEthItems = settings.grailType === GrailType.Each || settings.grailType === GrailType.Normal;
  const runesStats =  computeSubStats(items, ethItems, template.runes || null, null, settings, buildFlattenObjectCacheKey('runes', settings));
  const runewordsStats = computeSubStats(items, ethItems, template.runewords || null, null, settings, buildFlattenObjectCacheKey('runewords', settings));
  const armorStats = computeSubStats(items, ethItems, template.uniques.armor, ethTemplate.uniques.armor, settings, buildFlattenObjectCacheKey('armor', settings));
  const weaponStats = computeSubStats(items, ethItems, template.uniques.weapons, ethTemplate.uniques.weapons, settings, buildFlattenObjectCacheKey('weapon', settings));
  const otherStats = computeSubStats(items, ethItems, template.uniques.other, ethTemplate.uniques.other, settings, buildFlattenObjectCacheKey('other', settings));
  const setsStats = computeSubStats(items, ethItems, template.sets, null, settings, buildFlattenObjectCacheKey('sets', settings));
